...
//23. to keep track of whether if the game has started or not, so that only call nextSequence() on the first keypress.
let started = false;
...
$(".btn").click(function () {
  ...
  //11. Add the contents of the variable userChosenColour created in step 9 to the end of this new userClickedPattern
...
function checkAnswer(currentLevel) {
  ...
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
    ...
    //30. Play this sound if the user got one of the answers wrong.
    playSound("wrong");

    //31. Apply the class to the body of the website when the user gets one of the answers wrong and then remove it after 200 milliseconds.
    $("body").addClass("game-over");
    setTimeout(function () {
      $("body").removeClass("game-over");
    }, 200);

    //32. Change the h1 title to "Game Over, Press Any Key to Restart" if the user got the answer wrong.
...
function nextSequence() {
  ...
  //22. update the h1 with this change in the value of level.
  $("#level-title").text("Level " + level);
...
function animatePress(currentColor) {
  ...
  // 17. use Google/Stackoverflow to figure out how you can use Javascript to remove the pressed class after a 100 milliseconds.
  setTimeout(function () {
...
function startOver() {
  //35. Reset the values of level, gamePattern and started variables.
  level = 0;
  gamePattern = [];
  started = false;
}
